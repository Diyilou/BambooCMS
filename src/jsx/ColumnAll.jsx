...
class ColumnList extends React.Component {
  ...
  render () {
    ...
    return (
      <ul className='bili-columnlist'>
        <li>
          <a href='javascript:void(0)' data-id='0' onClick={showSonColumn.bind(__this)}>顶级栏目</a>
        {
          columnnav.map(function (item, index) {
            return <a key={index} data-typename={item.typename} data-reid={item.reid} data-id={item.id} onClick={showSonColumn.bind(__this)} href='javascript:void(0)'>{item.typename}</a>
          })
        }
        </li>
        {
          this.props.topcolumn.map(function (item, index) {
            ...
            return <li key={index} id='clist{item.id}'>
                    <div>
                      <i data-typename={item.typename} data-reid={item.reid} data-id={item.id} onClick={showSonColumn.bind(__this)} className='ion-plus-round'></i>
                      <span>{item.ishidden == 0 ? '显' : '隐'}</span>
                      <span>{item.type == 0 ? '文' : item.type == 1 ? '商' : item.type == 2 ? '图' : item.type == 3 ? '视' : ''}</span>
                      <Link data-item={JSON.stringify(item)} onClick={showArticles} to={newcolumnurl}><span>{item.id}</span>{item.typename}</Link>
                    </div>
                    <div>
                      <a href='javascript:void(0)'>预览</a>
                      <Link data-id={item.id} data-type={item.type} onClick={addSonColumn} data-typedir={item.typedir} to='/column/addson'>增加子类</Link>
