    ...
    return (
      <div className='bili-articleindex'>
        <ul className='bili-articlenav'>
          ...
          <li><Link onClick={addArticle} to={addNewUrl}>添加新视频</Link></li>
        </ul>
        <table cellSpacing='0'>
          <thead>
            <tr>
          ...
          <tbody>
            {
              this.state.articlelist.map(function (item, index) {
                return <tr key={index}>
                  ...
                  <td><Link data-aid={item.id} onClick={updateArticle} to='/column/video/update'>{item.title}</Link></td>
                  ...
                  <td>
                    ...
                    <Link data-aid={item.id} onClick={updateArticle} to='/column/video/update'>更改</Link>
                    <a href=''>浏览</a>
                  </td>
                </tr>
              })
...
class Video extends React.Component {
  render () {
    return (
      <div>
        {this.props.children ? this.props.children : <VideoIndex/>}
      </div>
    )
  }
}
